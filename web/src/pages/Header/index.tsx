...
const ItemNotification: React.FC<IItemNotification> = ({ noti }) => {
  ...
  const update = async () => {
    ...
    try {
      const res = await notificationApi.updateNotificationsOnUsers({
...
export default function HeaderLayout() {
  ...
  const items: MenuProps["items"] = notifications.map((noti: INotification) => {
    return {
      ...
      style: !noti.notificationStatus.seen
        ? { marginBottom: "0.2rem", backgroundColor: "rgba(0, 0, 0, 0.05)" }
        : { marginBottom: "0.2rem" },
    };
  });
  ...
  return (
    <Header className="header">
      <div className="header_left">
        <img
          style={{ width: "38px", height: "auto", marginRight: "0.5rem" }}
