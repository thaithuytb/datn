...
export class NotificationService {
  ...
  async createNotification(dto: CreateNotificationDto) {
    const listUserId = await this.authRepository.getUserIdsInGardensOnUsers(
      dto.gardenId,
    );
    ...
    return this.prisma.$transaction(async (prisma) => {
      ...
      const createNotificationsOnUsers =
        await this.notificationRepository.createNotificationsOnUsers(
          dataCreateNotificationsOnUsers,
          prisma,
        );
